...
public class CarrosController {
	...
	public ResponseEntity<List<CarroDTO>> getByTipo(@PathVariable("tipo") String tipo) {
		...
		// Iterable<Carro> carros = service.getCarrosByTipo(tipo);
		// return StreamSupport.stream(carros.spliterator(), false).count() == 0 ?
		// ResponseEntity.noContent().build()
		// : ResponseEntity.ok(carros);
		// Convert Iterable to List
		// List<Carro> result = StreamSupport.stream(carros.spliterator(),
